...
*
*/
public class GenerateTotalChart{
	...
	private static XYDataset createDataset(String username) {
		...
		int count2 = hdo.getDecisionCount(username);
		ArrayList al2 = hdo.getAllDecisions(username);
		for(int i = 1; i <= count2; i ++){
			xy.add(i+count, hdo.getCarbonAccumulation(i)+dbo.getCarbonAccumulation(count));
		}
...
//		String username="stu001";
	...
	try {
		ChartUtilities.saveChartAsJPEG(new File("E:/EnergyService/EnergyService/WebRoot/img/totallineChart.jpg"), j, 800, 600);
	} catch (IOException e) {
